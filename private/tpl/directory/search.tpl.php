...
<div id="content">
    <div class="center_content_inner">
        ...
        <div id="body_content">
			<div class="titre_plugg"><?= (($data['label'])? $data['label'] : 'Annuaire' ) ?></div>
                ...
                <div class="item_infinity">
                    ...
                    <a class="infobull_div tooltip" data-tooltip="#html-content<?= $row['id_user'] ?>" href="/user?id=<?= $row['id_user'] ?>"></a>
                        ...
                        <div class="des_connais">
                            <strong><a href="/user?id=<?php echo $row['id_user'] ?>" data-tooltip="#html-content<?= $row['id_user'] ?>" class="tooltip"><?= \Api\GlobalUtil::formatterName($row['firstname'], $row['lastname']) ?></a></strong>
...
$('#content_infinity').infinitescroll(
	{
		...
		path:function(index) { return "?page=" + index + '<?= (($data['label'])? '&label='.$data['label'] : '' ) ?>' ; } 
	}, 
	function(newElements, data, url){
		$('.tooltip').darkTooltip({animation:'flipIn', gravity:'west', theme:'light'});
		intPretty() ;
	}
);
</script>
