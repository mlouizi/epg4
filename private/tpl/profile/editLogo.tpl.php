...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		$(document).ready(function(){
				...
				onComplete: function(file, response){					
					...
					switch (response) {
						...
						default:
							...
							path = "/uploads/logo/temp/" + response;
			...
			$('#save_logo, #save_logo1').click(function(){
				parent.document.getElementById('pp_close').style.display = 'none' ;
				$.ajax({
					type: "POST",
					url: "/save_logo",
					data: "image_path=" + $('#image_path').val() + "&x=" + $('#x').val() + "&y=" + $('#y').val() + "&w=" + $('#w').val() + "&h=" + $('#h').val()  ,
					success: function(data){
						selector = parent.document.getElementById('profile_logo');						
						setTimeout(function(){selector.setAttribute('src', '/uploads/logo/' + $('#image_path').val()) ;parent.document.location = "/edit_profile"; }, 500) ;						
					}
				});			
			}) ;
		});
    ...
    <body>
        <div class="jc-demo-box">
            <div class="titre_plugg"><?= \Lang\edit_logo ?></div>
            <div class="clear20"></div>
			<div id="dialog" style="display: none;">
	  			<div class="message error"></div>
			</div>            
            <div style="padding:10px">
                <form action="save_logo" method="post" >
                	<table width="100%">
                        ...
                        <tr>
                            <td>
                                <table>    	
                                    <tr>
                                    	<td>
                                        	<div id="div_preview" style="float: left ;overflow: hidden; width: 200px; height: 200px; ">
												<?php if(file_exists('uploads/logo/original/'.$profile['company_logo']) && $profile['company_logo']!='') : ?>
                                                <img id="preview" src="/uploads/logo/original/<?php echo $profile['company_logo'] ?>" class="content_image"/>
                                                <?php else : ?>
                                                <img id="preview" src="/uploads/logo/logo.png" class="content_image"/>
                                                <?php endif ?>
                                            </div>
                                      	</td>
                                 	</tr>
                                    <tr>
                                    	<td>
                                        	<div align="right" id="divSubmit1"  <?= (file_exists('uploads/logo/original/'.$profile['company_logo']) && $profile['company_logo']!='')? '' : 'style="display:none"' ?> >
                                                <input type="button" value="<?= \Lang\save ?>" id="save_logo" class="btn btn-large btn-inverse" />
                                            </div>
                                      	</td>
                                  	</tr>
                               	</table>           
                            ...
                            <td>
                            	<div id="response">
									<?php if(file_exists('uploads/logo/original/'.$profile['company_logo']) && $profile['company_logo']!='') : ?><img src="/uploads/logo/original/<?= $profile['company_logo'] ?>" id='cropbox' /><?php endif ?>
                             	</div>
                           	</td>
                        </tr>
                    </table>                
                    <input type="hidden" id="image_path" name="image_path" value="<?php echo $profile['company_logo'] ?>" />
